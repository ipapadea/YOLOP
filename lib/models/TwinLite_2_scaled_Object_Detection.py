...
class ESPNet2_Encoder_scaledExtendedDWS(nn.Module):
    ...
    def forward(self, input):
        ...
        return (output1_0, output2_0, output2_cat, n6), (output0_cat, output1_cat, output2_cat)
...
class TwinLiteNet_2_scaled__(nn.Module):
    def __init__(self, p=2, q=3, scale=1.0):
        ...
        self.detector = IDetect(1, [[4.15629,  11.41984,  5.94761,  16.46950,  8.18673,  23.52688], [12.04416,  29.51737, 16.35089,  41.95507, 24.17928,  57.18741], [33.29597,  78.16243, 47.86408, 108.28889, 36.33312, 189.21414], [73.09806, 144.64581, 101.18080, 253.37000, 136.02821, 408.82248]], [64, 128, 256, 512])
    def forward(self, input):
        x = self.encoder(input)
        segmentation_heads = x[0]
        od_head = x[1]

        detector = self.detector(od_head)
        classifier1 = self.classifier_1(segmentation_heads[0])
        classifier2 = self.classifier_2(segmentation_heads[1])

        return (detector, classifier1, classifier2)
...
# model = TwinLiteNet_2_scaled__(scale=1.0)
