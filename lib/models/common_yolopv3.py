...
class DepthSeperabelConv2d(nn.Module):
    ...
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, downsample=None, act=True):
        ...
        self.depthwise = nn.Conv2d(inplanes, inplanes, kernel_size, stride=stride, groups=inplanes, padding=1,
                                   bias=False)
...
#
#         # --- SPP on C5 ---
#         self.spp = SPP(C5, C5)  # your own SPP implementation
#
#         # --- Top-down path ---
#         self.reduce_c5 = nn.Conv2d(C5, C4, 1, bias=False)
#         self.reduce_c4 = nn.Conv2d(C4, C4, 1, bias=False)
#         self.reduce_c3 = nn.Conv2d(C3, C3, 1, bias=False)
#
#         self.c4_td = DepthSeperabelConv2d(C4, C4)
#         self.c3_td = DepthSeperabelConv2d(C3, C3)
#
#         # --- Bottom-up path ---
#         self.c3_bu = DepthSeperabelConv2d(C3, C3)
#         self.c4_bu = DepthSeperabelConv2d(C4, C4)
#         self.c5_bu = DepthSeperabelConv2d(C5, C5)
#
...
#         p4 = self.reduce_c4(c4) + F.interpolate(p5, scale_factor=2, mode='nearest')
#         p4 = self.c4_td(p4)
#
#         p3 = self.reduce_c3(c3) + F.interpolate(p4, scale_factor=2, mode='nearest')
#         p3 = self.c3_td(p3)
#
#         # --- Bottom-up ---
#         p3_out = self.c3_bu(p3)
#         p4_out = self.c4_bu(p4 + F.max_pool2d(p3_out, 2))
#         p5_out = self.c5_bu(p5 + F.max_pool2d(p4_out, 2))
#
#         return [p3_out, p4_out, p5_out]

class LitePAN(nn.Module):
    def __init__(self, channels):  # channels = [64, 128, 256]
        super(LitePAN, self).__init__()
        C3, C4, C5 = channels

        # SPP: compress C5 → 256 (you can tune this too)
        self.spp = SPP(C5, 256)

        # Top-down
        self.reduce_c5 = nn.Conv2d(256, 128, 1, bias=False)
        self.reduce_c4 = nn.Conv2d(C4, 128, 1, bias=False)
        self.reduce_c3 = nn.Conv2d(C3, 128, 1, bias=False)

        self.c4_td = DepthSeperabelConv2d(128, 128)
        self.c3_td = DepthSeperabelConv2d(128, 128)

        # Bottom-up
        self.c3_bu = DepthSeperabelConv2d(128, 128)
        self.c4_bu = DepthSeperabelConv2d(128, 128)
        self.c5_bu = DepthSeperabelConv2d(128, 128)

    def forward(self, c3, c4, c5):
        # Apply SPP to highest-level feature
        c5 = self.spp(c5)  # [B, 256, H/32, W/32]

        # --- Top-down path ---
        p5 = self.reduce_c5(c5)  # → 128
        p4 = self.reduce_c4(c4) + F.interpolate(p5, scale_factor=2, mode='nearest')
        p4 = self.c4_td(p4)
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=13, anchors=(), ch=()):  # detection layer
        ...
        self.register_buffer('anchor_grid',
                             a.clone().view(self.nl, 1, -1, 1, 1, 2))  # shape(nl,1,na,1,1,2)   # shape(5,1,3,1,1,2)
        self.m = nn.ModuleList(
            nn.Conv2d(x, self.no * self.na, 1) for x in ch)  # output conv   # Channel = [128, 256, 512, 768, 1024]
